class AIGroupChat {
    ...
    initializeElements() {
        ...
        this.participantsContainer = document.getElementById('participants-container');
        this.addParticipantBtn = document.getElementById('add-participant-btn');
        this.createConversationBtn = document.getElementById('create-conversation-btn');
    ...
    setupSocketListeners() {
        this.socket.on('conversation-created', (data) => {
            ...
            if (data.success) {
                ...
                this.loadConversations();
                this.showNotification('Conversation created! Starting discussion...', 'success');
            } else {
    ...
    setupChatInterface(data) {
        ...
        this.updateMessageCounter(data.conversation.message_count, data.conversation.message_limit);
...
hideSetupModal() {
    ...
    addParticipant() {
        ...
        participantCard.innerHTML = `
            ...
            <div class="form-group">
                ...
                <button type="button" class="generate-personality-btn">Generate with AI</button>
            </div>
        `;
        
        const generateBtn = participantCard.querySelector('.generate-personality-btn');
        generateBtn.addEventListener('click', () => this.generatePersonality(participantCard));
    ...
    async generatePersonality(participantCard) {
        ...
        const personalityShortDetails = participantCard.querySelector('.personality-short-details');
        const personalityDetails = participantCard.querySelector('.system-instruction');
        
        if (!modelSelect.value || !personalityName.value) {
            this.showNotification('Please select a model and enter a personality name first', 'warning');
            return;
        }
    ...
    async createConversation() {
        ...
        const participants = [];
        const participantCards = this.participantsContainer.querySelectorAll('.participant-card');
        
        for (const card of participantCards) {
            const modelSelect = card.querySelector('.model-select');
            const personalityName = card.querySelector('.personality-name').value.trim();
            const personalityShortDetails = card.querySelector('.personality-short-details').value.trim();
            const personalityDetails = card.querySelector('.system-instruction').value.trim();
            
            if (!modelSelect.value || !personalityName) {
                this.showNotification('Please complete all participant information', 'warning');
                return;
            }
        ...
        if (participants.length === 0) {
            this.showNotification('Please add at least one AI participant', 'warning');
            return;
        }
        
        this.showLoading();
        console.log('Creating conversation with participants:', participants.length);
